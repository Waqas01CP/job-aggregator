...
| 2026-09-24 | Built: `src/contract.py`, daily at 06:30 UTC in its own workflow, sharing the fetch's concurrency group. Each adapter declares the fields it consumes in `CONSUMED`, and a test holds that list to what `parse()` reads. A field's shape is recorded as words across every posting in the response: present in all, some or none; null never, in some or always; and its type names. The fingerprint is `contract/fingerprint.json` on the data branch | Implementation facts, recorded by the implementing seat on the operator's go of 2026-09-24. Words rather than counts, because counts move daily with the postings, which is this record's own noise assumption. The Confirmation was met offline and once against the live boards with `--no-commit`: a field removed by hand from the stored fingerprint was named with its current shape. Whether `some` and `always` flip on a mostly-null field is not yet observed |
