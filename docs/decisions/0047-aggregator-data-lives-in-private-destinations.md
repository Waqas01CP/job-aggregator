...
| 2026-09-24 | When the private store cannot be restored, the public rows still reach the display and only the aggregator rows are withheld, counted in the run log. The run is marked failed at once, after its push | The operator's decision, D9: "jobs should and always must update its public row even if the private store fails", and a failure should reach him so it can be fixed. It replaces the seat's first design, which failed the whole projection, so one expired token froze the display. The aggregator rows are withheld because the private outcome stores that would keep a retired one out are unknown; a group mixing a public row with an aggregator row retired only in the private store would return, but ADR-0001's key carries each source's own publication date, which is why the Motive role sits in `Jobs` twice rather than merged, so such a group is not expected; none has been looked for. The notice is GitHub's failed-run email, which D2 ruled is not a notification system |
