...
**We will fail a run visibly when the private store cannot be written.** An unreachable private store must not degrade into a run that reports success having silently dropped its aggregator rows, which is the failure this record exists to end.
...
**The check that can fail:** revoke or corrupt the token and run. The run must fail and say so. If it completes and reports a successful fetch with no aggregator rows stored, the visible-failure rule is not implemented and the original defect has been rebuilt in a new place.
