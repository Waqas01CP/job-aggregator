...
topic: display
description: Superseded by ADR-0050, which consolidates this flow and its eight amendments. The operator classifies by setting one status in Jobs; the sweep copies the row out, then writes, verifies and deletes it at fifteen days. Supersedes ADR-0045.
date: 2026-09-19
...
**The operator classifies by setting `Status` on the row in `Jobs`.** *(Renamed 2026-09-24: the values are the table names, `rejected-not-a-fit`, `rejected-poor-filtering` and `accepted`. See Changes.)* Its values are `not fit`, `poor filtering`, `accepted` and `expired_before_review`, the last of which only the pipeline sets. *(Corrected 2026-09-23: `expired_before_review` is retired and the pipeline never writes `Status` at all. The three operator-set values stand. See Changes.)* Empty means not yet reviewed, and the "To review" view shows exactly those rows.
...
| 2026-09-24 | The three values are renamed to the tables they feed: `rejected-not-a-fit`, `rejected-poor-filtering` and `accepted`. `not fit` and `poor filtering` never existed in the base | The operator's decision, 2026-09-24. A value that names its table cannot be read as a judgement separate from where the row goes, and `applied` is retired because shortlisted and applied are held by `Stage` in the `accepted` table. He renamed the four choices in place rather than recreating them, which the connector cannot do: `update_field` accepts only a formula. Verified through the connector by the chat on 2026-09-25, both tables, with `expired_before_review` gone. The eight rows already marked kept their meaning and not one `Classified at` moved, so a rename does not restart the clock |
| 2026-09-25 | The budget is measured rather than inferred. The projection costs 5 calls a run at 44 groups, and about 300 a month at today's volume; with the sweep built, about 570, or 57% of the allowance | Production's first projection, 2026-09-24T03:27Z, spent 5 calls for 44 groups (29 public and 15 aggregator), and two test runs spent 6 each at 54 and 56 groups. The chat reproduced the group counts from `filtered.json` at `data` head `def4f935` on 2026-09-25: 39 groups over 345 rows, 334 admitted by the current chain, 29 groups, 3 calls for the public rows alone. The figure now moves with the aggregator count, because the private store makes those rows persist and be re-sent every run, and at about 100 aggregator groups the allowance is reached. **This is the eighth row: the next amendment supersedes this record**, per the Decision Record Standard |
| 2026-09-25 | Superseded by ADR-0050 | Eight amendments in six days, which the Decision Record Standard makes a supersession trigger, and two more were due: the copy step moving to every fetch, and the operator's D3 rule for closed postings, which this record has no place for. ADR-0050 carries every clause still in force and names what it carried, so nothing is retired by the move. The text here stays as it was, because it says what was believed on 2026-09-19 |
