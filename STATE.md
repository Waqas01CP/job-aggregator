...
**Waiting on the operator, as of 2026-09-17 night.** Pushing `be99ff8`, `bac377c` and the commit carrying this line. **Two trials run until 2026-09-24:** Himalayas stays on the board list for a week before the operator decides, and Banyan Canopy stays because one of its seven postings on 2026-09-11 was an AI and data role. On GitHub the Himalayas rows are discarded, so the week yields run-log counts only; seeing the roles needs a local run, about 36 requests, which is the operator's call. Whether to add AI terms that match nothing on today's boards. The Airtable route and then the token and secrets. The contract check. The architecture chat's answers. Done on the operator's instruction: the role-family order, recorded in the pool; ten software terms; the seniority rule with II and III excluded after checking and architect kept; Speechify and CodeRoad kept.
