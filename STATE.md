...
**The vertical slice has run successfully on GitHub, and the public `data` branch exists.** Manual run 35236531478 on 2026-09-17 at `6476573` fetched all twelve boards, kept 21 employer-board rows and 21 Himalayas rows, and pushed `data` with Greenhouse and Lever records only. The ticked test-mode run before it, 35236457737, failed its own tests because `TEST_MODE` reached the test step; the fix is committed locally and not pushed. The earlier scheduled run, 35179218050, failed at the commit on the old code. The Airtable display layer does not exist, so ADR-0009's slice Confirmation cannot yet be met.
...
| Tests independent of `TEST_MODE`, and `TEST_MODE` scoped to the Fetch step | DONE | [VERIFIED] run 35236457737 failed seven `main()` tests because the job-level `TEST_MODE=1` reached the test step; reproduced locally and in the extended simulation; 252 tests now pass with it unset, `1` and `0`; 5 mutations caught. **Committed, not pushed** | 2026-09-17 | `2026-09-17-first-github-runs.md` |
...
**Waiting on the operator, as of 2026-09-17.** Pushing the `TEST_MODE` fix, then a ticked manual run to exercise `data-test`. Whether Himalayas stays in `config/boards.json`: every production run now spends 25 of its 36 requests on it and keeps nothing. Whether `data/test/` may be moved aside. The title pool, the experience threshold's value, and the board list (Speechify, CodeRoad) are the operator's, not the chat's; the material for them is in `data/reports/title-pool-review-2026-09-17.md`, local only. The architecture chat's answers, which the operator will bring back.
...
**The workflow's branch fetch and push behave on GitHub as they did in simulation.** Partly [VERIFIED] by run 35236531478: with no `data` branch on origin, the first push created it, the identity worked, and `DATA_BRANCH` resolved to `data` for an unticked run. Still [BELIEVED]: fetching an existing branch, the restore, and a fast-forward push after a depth-1 fetch, which the next run of either kind exercises; and `DATA_BRANCH` resolving to `data-test` for a ticked run, which the ticked run after the push exercises. `2026-09-17-first-github-runs.md`.
