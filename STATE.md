...
**Last verified against `main` at `be99ff8`, one commit ahead of `origin/main` at `3ef83ba`, plus the commit that carries this line, 2026-09-17**, for the Headline, the Blocked section, the Known unverified entries dated 2026-09-17, the Pipeline rows, the README row, the title pool rows, and the Tooling rows added on 2026-09-17. Other Documentation and Tooling rows are carried forward from earlier verifications and were not rechecked.
...
**Waiting on the operator, as of 2026-09-17 night.** D1: whether Himalayas stays, after reading what it surfaces (evidence in `2026-09-17-operator-decisions-d1-to-d5.md`). D3: confirming the stated understanding of role families and their order, which terms to add, and whether a title seniority rule should exist and with which words. D5: keep or remove Speechify, CodeRoad and Banyan Canopy, now asked plainly. Airtable: which route to build the table, then the token and GitHub secrets. The contract check: whether, how often, how it reports. The architecture chat's answers. Done on the operator's instruction: `data/test/` moved to `data/archive/2026-09-16-test/`, every file verified identical; `forward deployment` added; the 39 unmatched terms kept and recorded; the experience rule deferred.
