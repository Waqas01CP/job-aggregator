...
| Weekly outcome sweep, routing to three outcome stores | **BLOCKED** | Blocked behind the writer, and so behind the same token. The sweep reads Airtable, which needs the token. Its design is now settled: ADR-0043 routes each swept row to exactly one of three append-only stores, and `expired_before_review` to none | 2026-09-18 | ADR-0014, ADR-0043 |
...
**Waiting on the operator, decided by him and not by a seat: whether `CHAT_STATE.md` is published.** The architecture chat removed its `.gitignore` line and its skip in `tools/generate_map.py`, with a comment saying it is now committed. Both were restored and neither is committed, because this repository is public, a push is not reversible, and the file's own description says it records the errors that chat made. The setup is deliberate and documented; publishing is still his call to make explicitly. Say the word and it goes in, frontmatter and map entry included, in one commit.
...
**Blocked on the operator, and named as a blocker rather than listed as unbuilt.** **The Airtable personal access token and the four repository secrets.** The writer cannot be built without them, and the sweep, the three outcome stores and the priority star all sit behind the writer. The token needs scopes `data.records:read`, `data.records:write` and `schema.bases:read`, limited to the "Job aggregator" base; the secrets are `AIRTABLE_TOKEN`, `AIRTABLE_BASE_ID`, `AIRTABLE_TABLE_ID` and `AIRTABLE_TEST_TABLE_ID`. The MCP authorisation that built the schema is a session's OAuth and the pipeline never uses it, so it does not substitute.
