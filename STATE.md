...
**Last verified against `main` at `bac377c`, two commits ahead of `origin/main` at `3ef83ba`, plus the commit that carries this line, 2026-09-17**, for the Headline, the Blocked section, the Known unverified entries dated 2026-09-17, the Pipeline rows, the README row, the title pool and seniority rows, the Himalayas and Banyan Canopy trials, and the Tooling rows added on 2026-09-17. Other Documentation and Tooling rows are carried forward from earlier verifications and were not rechecked.
...
**The vertical slice runs on GitHub in both modes, and the public `data` branch exists.** Manual production run 35236531478 on 2026-09-17 at `6476573` pushed `data` with Greenhouse and Lever records only. Manual test run 35239033514 at `3ef83ba` pushed `data-test` and left `data` untouched. No scheduled run has succeeded yet; the only scheduled run so far, 35179218050, failed at the commit on the old code. Not yet exercised on GitHub: a run that finds an existing branch, restores from it and pushes on top. The Airtable display layer does not exist, so ADR-0009's slice Confirmation cannot yet be met.
...
**The workflow's branch fetch and push behave on GitHub as they did in simulation.** Partly [VERIFIED]: run 35236531478 created `data` and run 35239033514 created `data-test`, so the identity, the push of a new branch, and `DATA_BRANCH` resolving both ways all work on GitHub. Still [BELIEVED]: fetching an existing branch, the restore, and a fast-forward push after a depth-1 fetch. The next scheduled run exercises all three against `data`. `2026-09-17-test-run-verified-and-no-input-work.md`.
