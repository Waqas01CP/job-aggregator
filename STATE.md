...
**Last verified 2026-09-23T12:15Z by the implementing seat, against `main` at `8f4c251` and `data` at `def4f93`.** Checked this session: the push state (`git ls-remote --heads origin` puts `main` at `8f4c251`, nothing unpushed before this commit), 383 tests passing, `MAP.md` current, six hook gates enabled, the Actions history (15 runs, 13 succeeded, the 2 failures both on 2026-09-17), the thirteen run logs on `data`, and the Airtable `Status` and `Classified at` fields through the connector. Everything else below carries the date of its own verification. `2026-09-22-speechify-rotation-and-the-writer-check.md`.
...
**The heredoc guard catches the failure it was built for.** Still never observed. [VERIFIED] 2026-09-23 that the harness invokes it: with the shell left in `docs/decisions`, a Bash call was refused with `python: can't open file '...\docs\decisions\tools\heredoc_guard.py'`. **That is a defect.** The hook passes a path relative to the working directory, Python exits 2 when it cannot open a script, and exit 2 from a PreToolUse hook blocks the call, so the guard fails closed on every Bash call once the shell leaves the repository root. Not fixed: anchoring the path on `CLAUDE_PROJECT_DIR` is proposed to the operator. `2026-09-22-speechify-rotation-and-the-writer-check.md`.
