...
**Last verified against `main` at `3ef83ba`, which is also `origin/main`, plus the commit that carries this line, 2026-09-17**, for the Headline, the Blocked section, the Known unverified entries dated 2026-09-17, the Pipeline rows, the README row, and the Tooling rows added on 2026-09-17. Other Documentation and Tooling rows are carried forward from earlier verifications and were not rechecked.
...
**Waiting on the operator, as of 2026-09-17 evening.** Decisions: whether Himalayas stays in `config/boards.json` (every production run spends 25 of its 36 requests on it and keeps nothing); whether `data/test/` may be moved aside; the title pool; the experience threshold's value; the board list (Speechify, CodeRoad). The material is in `data/reports/decision-pack-2026-09-17.md`, local only. The Airtable account, base, token and GitHub secrets, which the operator is setting up. Whether to build ADR-0018's contract check, how often, and how it reports. The architecture chat's answers, which the operator will bring back.
