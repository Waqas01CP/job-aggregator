...
**Last verified against `main` at `57f9382`, two commits ahead of `origin/main` at `bac377c`, plus the commit that carries this line, 2026-09-17**, for the Headline, the Blocked section, the Known unverified entries dated 2026-09-17, the Pipeline rows, the README row, the title pool and seniority rows, the Himalayas and Banyan Canopy trials, the first scheduled run, and the Tooling rows added on 2026-09-17. Other Documentation and Tooling rows are carried forward from earlier verifications and were not rechecked.
...
| Seniority rule in the filter chain, after the title rule | DONE | [VERIFIED] 303 tests pass, also with `TEST_MODE=1`. On the 796 production postings, pool version 3 with this rule keeps 267, of which 241 are Speechify's city copies of one role, drops 32 for seniority and 497 for title; 6 kept rows mention Pakistan. **Not pushed, so GitHub runs still use version 1 of the pool until it is** | 2026-09-17 | `2026-09-17-families-seniority-and-trials.md` |
...
**Waiting on the operator, as of 2026-09-18.** Pushing `ffccf8f`, `57f9382` and the commit carrying this line; until then GitHub runs pool version 2 without the seniority rule. **Himalayas visibility:** a week's trial shows counts only, because ADR-0020 keeps its rows off the branch and a runner discards them. Seeing the roles needs a local fetch, about 25 requests for Himalayas alone or 36 for every board, which is the operator's call; the alternative is the architecture chat's answer on whether its rows may reach the private Airtable base. **Airtable:** the plugin's tools reach only a new session, so the schema build starts there; then the token and the GitHub secrets. **The contract check:** whether, how often, how it reports. The architecture chat's answers. Banyan Canopy's review on 2026-09-24. Done on the operator's instruction: pool version 4's AI terms; the family order; the seniority rule; Speechify and CodeRoad kept.
